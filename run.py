...
from config import *
from common.common import DelReport, ErrorExcep, logger
...
class RunPytest:
    ...
    def run_debug():
        ...
        DelReport().run_del_report()
        # 生成测试结果目录
        PRPORE_JSON_DIR, PRPORE_ALLURE_DIR = RunPytest.output_path('testport')
        pytest.main(
            ['-m', 'test_business', '-n=1', '--reruns=0', '--alluredir', f'{PRPORE_JSON_DIR}', f'{CASE_DIR}'])
        ...
        time.sleep(3)
        # 自动打开 Allure 报告
        os.system(f'allure open {PRPORE_ALLURE_DIR}')
...
# python run.py all 4 1 demo True a  这将运行所有测试用例 (all)，使用 4 个线程 (4)，失败重跑 1 次 (1)，生成结果目录为 demo，开启邮件发送 (True) 并启用企业微信和钉钉消息通知 (a)。
# adopts 参数说明
