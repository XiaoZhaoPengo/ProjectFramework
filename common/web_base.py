...
from common.common import ErrorExcep, logger, is_assertion, reda_conf
from common.reda_data import GetCaseYmal, replace_py_yaml
...
class Operation(Enum):
    """
       ...
       web_forward                 >               前进
       web_refresh                 >               刷新
       web_all_handle              >               获取所有句柄
       web_current_window          >               获取当前句柄
       web_switch_windows          >               切换句柄
       screen_shot                 >               截图
       web_url                     >               获取当前url
       web_title                   >               获取当前title
...
class Base:
    ...
    def switch_to_latest_window(self) -> EM or None:
        """
        切换到最新打开的窗口
        :return:
        """
        ...
        if all_handles:
            ...
            logger.debug(f'切换到最新窗口{latest_handle}')
            return self.driver.switch_to.window(latest_handle)
        else:
            logger.debug("没有打开的窗口")
            return None
    ...
    def web_select_locat(self, types: str, locate: str, value: str) -> None:
        """
    ...
    def often_click(self, types: str, locate: str, index: int = None) -> None:
        """
        根据定位类型和定位表达式获取元素，并执行点击操作。
        如果指定了索引，则从元素列表中获取对应索引的元素并点击。
        :param types: 定位类型
        :param locate: 定位元素
        :param index: 列表索引位置  find_element传递时 此值必填
        :return:
        """
        if index is not None:
            # 如果有索引，说明是操作元素列表中的特定元素
            elements = self.driver_element(types=types, locate=locate)  # 假设 driver_elements 返回元素列表
            if 0 <= index < len(elements):
                elements[index].click()
                logger.info(f"点击了定位表达式 {locate} 对应的元素列表中的第 {index + 1} 个元素")
            else:
                logger.error(f"索引 {index} 超出范围，无法点击元素")
        else:
            # 没有索引，直接操作单个元素
            element = self.driver_element(types=types, locate=locate)
            if element:
                element.click()
                logger.info(f"点击了定位表达式 {locate} 对应的元素")
            else:
                logger.error(f"无法找到并点击定位表达式 {locate} 对应的元素")
...
class Web(Base):
    ...
    def web_judge_execution(self, types: str, locate: str, operate: str = None, text: str = None, notes: str = None, index: int = None, wait: float = None, use_js: bool = False) -> Any:
        """
          操作类型:
        操作类型                                    执行动作
        input                       >               输入
        ...
        if operate is None:
            ...
            if operate == 'click':  # 点击操作
                try:
                    # 总是先尝试使用 Selenium 的点击方法
                    self.often_click(types=types, locate=locate, index=index)
                    self.sleep(wait)
                    logger.info(f"成功将点击操作到 {locate}（{types}）定位的元素中")
                    logger.debug(notes)
                    
                    # 如果不需要使用 JavaScript，则直接返回
                    if not use_js:
                        return True  # 或者返回其他适当的值
                
                except Exception as e:
                    # 如果 Selenium 的点击方法抛出异常，且 use_js 为 True，则尝试使用 JavaScript
                    if use_js:
                        try:
                            if types == 'class':
                                js_code = f"document.querySelector('.{locate}').click();"
                            elif types == 'xpath':
                                # 注意：XPath 需要特殊处理
                                # 这里我们使用一个假设的 XPath 转换函数，实际情况中需要实现这个函数
                                # 或者考虑将 XPath 转换为 CSS 选择器（如果可能）
                                # 这里为了简化，我们只记录一个警告
                                logger.warning(f"XPath 不直接支持通过 JavaScript 高效点击，但尝试执行。")
                                # 假设我们有一个函数可以将 XPath 转换为可执行的 JS 代码
                                # js_code = convert_xpath_to_js_click(locate)
                                # 由于这里不实现转换，我们仅作记录
                                js_code = "尝试执行 XPath 点击，但此处未实现转换逻辑。"
                                # 实际上，你应该在这里处理 XPath，或者改用其他方法定位元素
                            
                            # 如果 js_code 确实准备好了，就执行它
                            if js_code.strip() != "":
                                self.driver.execute_script(js_code)
                                self.sleep(wait)
                                logger.info(
                                    f"使用 JavaScript 尝试点击定位的元素: {locate}（{types}），但 XPath 处理可能需要额外注意。")
                                logger.debug(notes)
                                return True  # 或者返回其他适当的值
                            else:
                                logger.error(f"无法为 XPath 生成有效的 JavaScript 点击代码。")
                        
                        except Exception as js_e:
                            logger.error(f"JavaScript 点击失败: {js_e}")
                    
                    # 如果 JavaScript 点击也失败（或未执行），则重新抛出原始异常
                    raise e
                
                # 如果不需要 use_js 或者 use_js 为 False 但 Selenium 点击成功，则直接返回
                return True  # 或者返回其他适当的值
    ...
    def webexe(self, yamlfile, case, text=None, wait=0.1):
        ...
        try:
            yaml = replace_py_yaml(yamlfile)  # 假设这个函数用于处理或加载YAML文件
            locator_data = self.get_case(yaml, case)  # 假设这个函数用于从YAML中获取用例数据
            locator_step = locator_data.stepCount()
            
            if locator_step is None or not isinstance(locator_step, int):
                print(f"Warning: No steps found for case '{case}'. Skipping execution.")
                return relust
            ...
            for locator in range(locator_step):
                ...
                try:
                    if operate in ('input', 'clear_continue_input', 'jsclear_continue_input'):
                        self.web_judge_execution(types=locator_data.types(locator), locate=locator_data.locate(locator),
                                                 operate=operate, notes=locator_data.info(locator),
                    ...
                    print(f"Error executing step {locator + 1}: {e}")
                    break  # 或者根据需要处理异常，而不是直接跳出循环
                
                # 更新等待时间
                if waits is not None:
                    wait = waits
                
                self.sleep(wait)  # 假设这是一个等待方法，比如 time.sleep 的封装
        
        except Exception as e:
            print(f"Error loading or executing case '{case}': {e}")
...
class AutoRunCase(Web):
    ...
    def run(self, yamlfile, case, test_data=None, forwait=None):
        """
        自动执行定位步骤  使用run 函数时 test_data 直接传递为可迭代对象
        :param yamlfile:  yaml文件
        :param case: yaml定位用例
        ...
        for locator in range(locator_step):
            ...
            if locator_data.operate(locator) in ('input', 'clear_continue_input', 'jsclear_continue_input'):
                ...
                self.web_judge_execution(types=locator_data.types(locator), locate=locator_data.locate(locator),
                                         ...
                                         text=test_data[locator], index=locator_data.listindex(locator),
