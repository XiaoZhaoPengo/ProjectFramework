...
from common.common import ErrorExcep, logger, reda_conf
from common.db import RedisPool
...
# 读取yaml数据
class GetCaseYmal:
    def __init__(self, yaml_name: str, case_name: str = None, file_path: str = None):
        IS_REDIS = reda_conf('CURRENCY').get('IS_REDIS')
        self.isredis = IS_REDIS
        self.yaml_name = yaml_name
        self.case_name = case_name

        if file_path is not None:
            self.FLIE_PATH = file_path
        else:
            if case_name is not None:
                self.modelname = yaml_name
    ...
    def open_yaml(self):
        # 如果文件路径以 .yamlc 结尾，则替换为 .yaml
        if self.FLIE_PATH.endswith('.yamlc'):
            self.FLIE_PATH = self.FLIE_PATH[:-1]  # 去掉 'c'，转换为 .yaml 文件

        if not os.path.exists(self.FLIE_PATH):
            logger.error(f"文件不存在: {self.FLIE_PATH}")
            raise FileNotFoundError(f"文件不存在: {self.FLIE_PATH}")
        ...
        try:
            ...
            logger.error(f'Error opening ymal file: {e}')
            raise
    ...
    def get_yaml(self):
        ...
        if yaml_data is not None:
            if isinstance(yaml_data, list) and len(yaml_data) > 1:
                ...
                logger.error('YAML 文件不包含有效的测试用例数据。')
                raise ValueError("YAML 文件不包含有效的测试用例数据。")
        else:
            logger.error('YAML 文件为空或无法加载。')
            raise ValueError("YAML 文件为空或无法加载。")
